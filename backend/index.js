...
// Tushare行情接口 - 路径参数版本 (for frontend direct navigation)
app.get('/api/tushare/quote/:code', async (req, res) => {
  try {
...
app.get('/api/stock/:code', async (req, res) => {
  try {
    ...
    // 查询数据库获取基本信息
    const [stocks] = await pool.execute(`
      SELECT code, name, price, change_percent, industry, market
      FROM stocks 
      WHERE code = ?
    `, [code]);
...
// BE-001: 获取股票基础信息 - /api/stock/basic/:code
app.get('/api/stock/basic/:code', async (req, res) => {
  try {
    const code = req.params.code;
    
    // 先尝试从 stocks 表查询（总会有数据）
    const [stocks2] = await pool.execute(`
      SELECT code, name, price, change_percent, industry, market
      FROM stocks 
      WHERE code = ?
    `, [code]);
    ...
    res.json({
      code: 200,
      basic_info: stocks2[0]
    });
  } catch (error) {
    console.error('===== 获取股票基础信息失败 =====');
...
// BE-001: 获取K线数据 - /api/stock/kline/:code
...
// BE-001: 获取资金流向/北向持股 - /api/stock/capital/:code
app.get('/api/stock/capital/:code', async (req, res) => {
  try {
    const code = req.params.code;
...
// 添加持仓 - 前端使用 /api/portfolio/holdings 端点
app.post('/api/portfolio/holdings', authenticateToken, async (req, res) => {
  try {
    // 前端参数名: symbol, quantity, cost_price
    const { symbol, quantity, cost_price, name } = req.body;
    
    // 参数映射到后端格式: stock_code, stock_name, quantity, avg_price
    const stock_code = symbol;
    const stock_name = name || symbol;
    const qty = parseInt(quantity);
    const avgPrice = parseFloat(cost_price);
    
    if (!stock_code || !stock_name || !qty || qty <= 0 || !avgPrice || avgPrice <= 0) {
      return res.status(400).json({ code: 400, message: '参数不完整' });
    }
    
    // 检查是否已存在
    // DB schema has user_id (int from JWT userId)
    ...
    if (existing.length > 0) {
      ...
      const newAvgPrice = ((existing[0].quantity * existing[0].avg_price) + (qty * avgPrice)) / newQuantity;
      ...
      await pool.execute(
        ...
        [userId, stock_code, stock_name, qty, avgPrice]
      );
    }
...
app.get('/api/ai/portfolio-analysis', authenticateToken, async (req, res) => {
  try {
    ...
    const result = {
      ...
      suggestion: '组合配置较为均衡，建议适当增加港股和美股配置分散风险'
    };
...
// AI个股诊断API - 火山引擎 Coding Plan
app.get('/api/ai/stock-diagnosis', async (req, res) => {
  let stock = null;
  try {
    const code = req.query.code;
    if (!code) {
    ...
    // 构建prompt
    const prompt = `你是一位专业的投资分析师，请对股票 ${stock.name}（${code}）进行诊断分析。股票当前价格 ¥${stock.price}，属于${stock.industry}行业。

请给出以下分析，使用JSON格式返回：
{
  "valuation": "低估|合理估值|高估",
  "risk_level": "low|medium|high",
  "health_score": 健康评分0-100,
  "trend": "上涨趋势|震荡整理|下跌趋势",
  "support": 支撑位价格数字,
  "resistance": 压力位价格数字,
  "suggestion": "你的投资建议"
}
`;
    ...
    const response = await axios.post('https://ark.cn-beijing.volces.com/api/v3/chat/completions', {
      model: 'ep-20240515163107-urkkvoxxxh',
      messages: [
        {
          role: 'user',
          content: prompt
    ...
    try {
      ...
      diagnosis = jsonMatch ? JSON.parse(jsonMatch[1]) : JSON.parse(content);
    } catch (parseError) {
      console.error('AI返回解析失败:', parseError);
      // 如果解析失败，使用兜底分析
      const pLevel = stock.price > 50 ? '高估' : stock.price > 20 ? '合理估值' : '低估';
      const rLevel = stock.industry === '银行' || stock.industry === '消费' ? 'low' : 
                   stock.industry === '科技' || stock.industry === '半导体' ? 'high' : 'medium';
      diagnosis = {
        valuation: pLevel,
        risk_level: rLevel,
        health_score: stock.price > 50 ? 50 : stock.price > 20 ? 65 : 80,
        trend: '震荡整理',
        support: parseFloat((stock.price * 0.95).toFixed(2)),
        resistance: parseFloat((stock.price * 1.05).toFixed(2)),
        suggestion: '当前估值适中，建议持有观望。'
      };
    }
...
async function start() {
  await initDB();
  app.listen(PORT, () => {
    console.log(`FamilyStock 后端服务已启动，监听端口 ${PORT}`);
  });
}
