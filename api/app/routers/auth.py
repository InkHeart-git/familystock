...
class RegisterRequest(BaseModel):
    ...
    id_card: Optional[str] = None     # 身份证号码
...
async def register(request: RegisterRequest):
    ...
    try:
        with conn.cursor() as cur:
            ...
            # 添加新字段（如果不存在）
            try:
                cur.execute("ALTER TABLE users ADD COLUMN IF NOT EXISTS real_name VARCHAR(100)")
                cur.execute("ALTER TABLE users ADD COLUMN IF NOT EXISTS id_card VARCHAR(18)")
            except Exception as e:
                print(f"添加字段失败（可能已存在）: {e}")
                conn.rollback()  # 回滚失败的ALTER TABLE事务
            ...
            id_card = request.id_card or ""
            cur.execute("""
                INSERT INTO users (phone, password_hash, name, real_name, id_card)
                VALUES (%s, %s, %s, %s, %s)
                RETURNING id, phone, name, real_name, id_card
            """, (request.phone, request.password, user_name, real_name, id_card))
